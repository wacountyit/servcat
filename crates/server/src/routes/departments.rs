...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/departments/{id}/logo", post(upload_logo).delete(remove_logo))
        .layer(DefaultBodyLimit::max(MAX_LOGO_UPLOAD_BYTES))
}

async fn list(_auth_user: AuthUser, State(state): State<AppState>) -> Result<Json<Vec<Department>>, ApiError> {
...
async fn create(
    ...
    Ok(Json(departments::create(&state.pool, &new_department).await?))
}
...
async fn delete(auth_user: AuthUser, State(state): State<AppState>, Path(id): Path<Uuid>) -> Result<(), ApiError> {
    auth_user.require_role(&[Role::Admin])?;
...
async fn upload_logo(
    ...
    let current = departments::get_by_id(&state.pool, id).await?.ok_or_else(|| ApiError::NotFound("department not found".into()))?;
    let logo_url =
        uploads::save_logo(&state.uploads_dir, "departments", &id.to_string(), content_type, body, current.logo_url.as_deref())
            .await?;
...
async fn remove_logo(auth_user: AuthUser, State(state): State<AppState>, Path(id): Path<Uuid>) -> Result<Json<Department>, ApiError> {
    auth_user.require_role(&[Role::Admin])?;
    let current = departments::get_by_id(&state.pool, id).await?.ok_or_else(|| ApiError::NotFound("department not found".into()))?;
    uploads::delete_logo(&state.uploads_dir, current.logo_url.as_deref()).await;

    let updated =
        departments::set_logo_url(&state.pool, id, None).await?.ok_or_else(|| ApiError::NotFound("department not found".into()))?;
    Ok(Json(updated))
}
