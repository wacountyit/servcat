...
async fn list_mine(
    ...
    Ok(Json(approvals_repo::list_pending_for_approver(&state.pool, auth_user.0.id).await?))
}
...
async fn decide(
    ...
    let approval =
        servcat_approvals::decide(&state.pool, id, auth_user.0.id, req.decision, req.comment).await?;

    let deps = orchestrator::Deps { pool: &state.pool, notifier: state.notifier.as_ref(), connectors: &state.connectors };
    let instance =
        orchestrator::resume_after_approval(&deps, approval.workflow_instance_id, &approval.step_id, req.decision)
            .await?;
