...
impl GlpiConnector {
    ...
    async fn init_session(&self) -> Result<String, ConnectorError> {
        ...
        body.get("session_token")
            ...
            .map(str::to_string)
            .ok_or_else(|| {
                ConnectorError::UnexpectedResponse(
                    "missing 'session_token' in GLPI response".into(),
                )
            })
    }
...
impl TicketConnector for GlpiConnector {
    ...
    async fn dispatch(&self, payload: &JsonValue) -> Result<DispatchResult, ConnectorError> {
        ...
        let result = async {
            ...
            let id = body.get("id").and_then(JsonValue::as_u64).ok_or_else(|| {
                ConnectorError::UnexpectedResponse("missing 'id' in GLPI response".into())
            })?;

            Ok(DispatchResult {
                external_ticket_id: id.to_string(),
                external_ticket_url: Some(format!(
